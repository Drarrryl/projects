...
while True:
    ...
    if level == 9: # Staircase into labrynth level
        ...
        # Modified Boss Movement
        boss.sety(boss.ycor() + bossdy)  # Move the boss based on bossdy
        if boss.ycor() > labFloor.ycor() + 250:
            bossdy *= -1  # Reverse direction when hitting top or bottom
        if boss.ycor() < labFloor.ycor() + 65:
            bossdy *= -1  # Reverse direction when hitting top or bottom
        if checkCollide(player, key):
            key.goto(boss.xcor(), boss.ycor())
            boss.sety(labFloor.ycor() + 249)
            bossdy = -1
